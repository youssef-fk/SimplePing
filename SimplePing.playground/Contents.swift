...
public protocol PingDelegate: class {
...
public func hostStringWithData(_ data: Data) -> String {
    ...
    defer {
        hostStrRef.deallocate(capacity: Int(maxHostLen))
    }
    let portStrRef = UnsafeMutablePointer<Int8>.allocate(capacity: Int(maxPortLen))
    defer {
        portStrRef.deallocate(capacity: Int(maxPortLen))
    }
...
public class SimplePing: Ping {
    ...
    private func pingPacketWithType(_ type: UInt8, payload: Data, requiresChecksum: Bool) -> Data {
        ...
        defer {
            icmpHeaderRef.deallocate(capacity: 1)
        }
        ...
        defer {
            packet.deallocate(capacity: packetSize)
        }
    ...
    private func icmpHeaderOffsetInIPv4Packet(_ packet: Data) -> Int {
        ...
        defer {
            ipHeaderRef.deallocate(capacity: 1)
        }
    ...
    private func validatePing4ResponsePacket(_ packet: inout Data, sequence: inout UInt16) -> Bool {
        ...
        defer {
            icmpDataRef.deallocate(capacity: icmpDataSize)
        }
        ...
        packet.replaceSubrange(Range<Data.Index>(0..<icmpHeaderOffset), with: [])
...
class PingVC: UIViewController {
    ...
    override func loadView() {
        // load view hierarchy
        self.view = UIView(frame: CGRect(origin: CGPoint.zero, size: CGSize(width: 375, height: 667)))
        ...
        addressField.backgroundColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
        addressField.placeholder = "Enter IP"
        addressField.text = "google.com"
        addressField.layer.cornerRadius = 3
        addressField.layer.masksToBounds = true
        
        pingButton = UIButton(type: .roundedRect)
        pingButton.setTitle("Ping", for: .normal)
        pingButton.layer.cornerRadius = 10
        pingButton.layer.masksToBounds = true
        pingButton.layer.borderColor = #colorLiteral(red: 1, green: 0.8871350884, blue: 0.4641724229, alpha: 0.7513204225)
        pingButton.layer.borderWidth = 1.0
        pingButton.addTarget(self, action: #selector(startPing), for: .touchUpInside)
        ...
        if #available(iOS 11.0, *) {
            addressField.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: basicInset).isActive = true
            pingButton.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: basicInset).isActive = true
        } else {
            addressField.topAnchor.constraint(equalTo: self.topLayoutGuide.topAnchor, constant: basicInset).isActive = true
            pingButton.topAnchor.constraint(equalTo: self.topLayoutGuide.topAnchor, constant: basicInset).isActive = true
        }
        NSLayoutConstraint.activate([
            addressField.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: basicInset),
            addressField.trailingAnchor.constraint(equalTo: pingButton.leadingAnchor, constant: -8),
            addressField.heightAnchor.constraint(equalTo: pingButton.heightAnchor),
            
            pingButton.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -basicInset),
            pingButton.heightAnchor.constraint(equalToConstant: 30),
            pingButton.widthAnchor.constraint(equalToConstant: 70),
            
            
            textView.topAnchor.constraint(equalTo: addressField.bottomAnchor, constant: 8),
            textView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -basicInset),
            textView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: basicInset)
            ])
        textFieldBottomConstraint = textView.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -basicInset)
    ...
    @objc private func keyboardDidShow(_ notification: Notification) {
        if let userinfo = notification.userInfo as? [String: Any],
            ...
            self.textFieldBottomConstraint.constant = -rect.size.height - self.basicInset
        }
    }
    
    @objc private func keyboardWillHide(_ notification: Notification) {
        self.textFieldBottomConstraint.constant = -self.basicInset
    }
    
    @objc private func startPing() {
        pingWillStart()
        addressField.resignFirstResponder()
        let ping = SimplePing(hostName: addressField.text!)
        ping.delegate = self
        ping.start()
    ...
    func pingWillStart() {
        DispatchQueue.main.async {
            ...
            self.pingButton.removeTarget(self, action: nil, for: .touchUpInside)
            self.pingButton.addTarget(self, action: #selector(self.stopPing), for: .touchUpInside)
        }
    }
    
    func pingDidStop() {
        DispatchQueue.main.async {
            self.pingButton.setTitle("Ping", for: .normal)
            self.pingButton.removeTarget(self, action: nil, for: .touchUpInside)
            self.pingButton.addTarget(self, action: #selector(self.startPing), for: .touchUpInside)
        }
    }
}

extension PingVC: PingDelegate {
    func pinger(_ pinger: Ping, didStartWithAddress address: String) {
        textView.insertText("Started ping to \(address)\n")
    ...
    func pinger(_ pinger: Ping, didFailWithError error: Error) {
        textView.insertText("failed: \(error.localizedDescription)\n")
        self.stopPing()
    }
    
    func pinger(_ pinger: Ping, didSendPacket packet: Data, sequence: UInt16) {
        self.sendTimes[sequence] = Date()
    }
    
    func pinger(_ pinger: Ping, didFailToSendPacket packet: Data, sequence: UInt16, error: Error) {
        textView.insertText("#\(sequence) send failed: \(error.localizedDescription)\n")
    }
    
    func pinger(_ pinger: Ping, didReceivePingResponsePacket packet: Data, sequence: UInt16, from: String) {
        let now = Date()
        let sendDate = sendTimes[sequence]!
        let ping = now.timeIntervalSince(sendDate) * 1000
        textView.insertText("#\(sequence) received from \(from), size=\(packet.count), ping=\(String(format: "%.3lf", ping))ms\n")
    }
    
    func pinger(_ pinger: Ping, didReceiveUnexpectedPacket packet: Data, from: String) {
        textView.insertText("unexpected packet from \(from), size=\(packet.count), \(packet.hex)\n")
    }
}
